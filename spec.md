...
- Directs `Editor.vue` to render dynamic inspector controls through the shared `InspectorField.vue` component (`text`, `textarea`, `toggle`, `theme-color`, `font-size`, `segmented`, `columns`, `range`, `number`, `select`, `media`, `repeater`). `ContentInspector.vue` groups fields into collapsible **Content** / **Style & Layout** sections (via each field's optional `group` key) plus a fixed **Effects** (scroll-reveal) section.
...
5. **Site search**: `navigation_config.header.search` (`show` boolean plus optional `placeholder`, toggled in the Buttons panel) renders a search icon in the header actions. Opening it shows a dropdown that filters the site's published pages by title or slug and links to them. When search is enabled, `TenantPublicSiteController` includes a `pages` prop of published `title`/`slug` pairs; when disabled, the list is omitted. It is page navigation only — not a product, content, or availability search.
6. **Regression coverage**: `TenantNavigationTest` verifies API persistence, search settings validation, the conditional published-pages payload, and confirms that saved navigation is present in both editor and public Inertia responses.
...
Every catalog layout begins with a composable `SectionBlock` hero. The catalog and editor share six pattern keys: `hero-centered`, `hero-split-right`, `hero-split-left`, `hero-editorial`, `hero-minimal`, and `hero-stats` (copy and actions beside three proof-point stat callouts). Each pattern is an ordinary nested block tree, not a second template runtime. Editable descendants carry a semantic `patternRole` in ordinary props (`eyebrow`, `heading`, `body`, `primaryAction`, `secondaryAction`, or `media`). The section inspector's **Change layout** action creates the selected composition with fresh IDs and transfers role-matched text, button labels/URLs, and image source/alt values before the normal autosave flow persists the result.
