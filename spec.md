...
Retail storefront composition uses the ordinary page block AST and the existing editor/save/publish/public-rendering pipeline. The registered blocks are Announcement, Image with Text, Collection List, Product Grid, Product Detail, Newsletter, and Store Values. Product and collection blocks contain editable placeholder data plus stable `sourceKey` and product `key` values for future provider hydration.
