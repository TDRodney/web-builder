...
const handleSubmit = async () => {
  if (isEditable) return; // Disable submission inside editor
  ...
  try {
    ...
    if (res.ok) {
      ...
      errorMessage.value = body.message || 'Submission failed. Please check inputs and try again.';
    }
  } catch (err) {
...
<template>
  <div class="contact-block-wrapper" :style="{ color: 'var(--theme-text, #0f172a)' }">
    <div v-if="showSuccess" class="success-banner" :style="{ borderRadius: 'var(--theme-border-radius, 6px)' }">
      ...
      <p class="success-text">{{ blockProps.successMessage || 'Thank you! Your message has been sent successfully.' }}</p>
      <button type="button" class="reset-btn" @click="showSuccess = false">Send another message</button>
    </div>
    ...
    <form v-else @submit.prevent="handleSubmit" class="contact-form">
      ...
      >
        {{ isSubmitting ? 'Sending...' : (blockProps.submitLabel || 'Send Message') }}
      </button>
    </form>
  </div>
</template>
