...
const isPopular = (plan) => {
    return plan.isPopular === true || plan.isPopular === 'yes';
};

const updateFeature = (plan, index, value) => {
    const features = getFeatures(plan.features);
    features[index] = value;
    plan.features = features.filter(Boolean).join(', ');
};
</script>
    ...
    >
        <div v-if="plansList.length" class="plans-grid">
            <div
                ...
                :key="index"
                class="plan-card"
                :class="{ 'plan-card-popular': isPopular(plan) }"
                :style="{
                    borderRadius: 'var(--theme-border-radius, 8px)',
                    borderColor: isPopular(plan)
                        ? 'var(--theme-primary, #6366f1)'
                        : 'rgba(255,255,255,0.06)',
                }"
            >
                <span v-if="isPopular(plan)" class="popular-badge"
                    >POPULAR</span
                >
                ...
                <ul class="plan-features">
                    <li
                        v-for="(feat, fIdx) in getFeatures(plan.features)"
                        :key="fIdx"
                    >
                        <svg
                        ...
                        <InlineText
                            ...
                            @update:value="updateFeature(plan, fIdx, $event)"
                        />
                    </li>
                </ul>
                ...
                <a
                    v-if="plan.ctaText"
                    :href="
                        isEditable ? 'javascript:void(0)' : plan.ctaUrl || '#'
                    "
                    class="plan-cta-btn"
                    :class="isPopular(plan) ? 'cta-popular' : 'cta-standard'"
                    :style="{
                        borderRadius: 'var(--theme-border-radius, 6px)',
                        backgroundColor: isPopular(plan)
                            ? 'var(--theme-primary, #6366f1)'
                            : 'transparent',
                        borderColor: 'var(--theme-primary, #6366f1)',
                    }"
                    @click="isEditable && $event.preventDefault()"
                >
                    <InlineText
                        v-model:value="plan.ctaText"
                        tag="span"
                        placeholder="Button label"
                        aria-label="Plan button label"
                    />
                </a>
            </div>
        </div>
